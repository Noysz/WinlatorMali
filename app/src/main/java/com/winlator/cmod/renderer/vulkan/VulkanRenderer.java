package com.winlator.cmod.renderer.vulkan;
...
public class VulkanRenderer implements WindowManager.OnWindowModificationListener,
    ...
    private void sendCursorToNative(Cursor cursor) {
        ...
        if (effVis && cd != null && cd.getData() != null) {
            synchronized (cd.renderLock) {
                nativeUpdateCursorImage(nativeHandle, cd.getData(), cd.width, cd.height, hotX, hotY);
                if (nativeMode) {
    ...
    public void onUpdateWindowContentDirect(Window window, Drawable pixmap, short xOff, short yOff) {
        ...
        if (!nativeMode && window.id == fpsWindowId) {
            if (classicHudRef != null && classicHudRef.isAttachedToWindow()) classicHudRef.postInvalidate();
        }
        synchronized (lock) {
    ...
    public void setNativeMode(boolean mode) {
        ...
        xServerView.post(() -> com.winlator.cmod.core.AppUtils.showToast(xServerView.getContext(), msg));
    }
    ...
    public void setWinlatorHUD(Object fr) {
        if (fr instanceof WinlatorHUD) classicHudRef = (WinlatorHUD) fr;
    }
